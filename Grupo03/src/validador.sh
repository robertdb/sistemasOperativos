...
tarjetaCorrecta() {
    es_numero='^[0-9]+$';
    if ! [[ $1 =~ $es_numero ]]; then
        rechazados="ERROR: TARJETA con caracteres que No son un números"
        return 1
    fi
...
yaseproceso() {
    archi=$1
    unset lineas
    for lineas in ./$ACEPTADOS/$PROCESADOS/*.txt; do
...
buscarEntidadBancaria() {
    ...
    oldIFS=$IFS;
    IFS=$'\n';
    for linea in $(cat ./$MAESTROS/bamae); do
        enti=$(echo "$linea" | cut -d ';' -f1);
        if [ $contador -ne 0 ] && [ $1 = $enti ]; then
...
grabarDatosDenunciadaBloqueada() {
    ...
    contar=0;
    encontro=0;
    for linea in $(cat ./$MAESTROS/tx_tarjetas); do
        #linea=$(echo -e "$line\n");
        cuenta=$(echo "$linea" | cut -d ';' -f2);
        if [ $contador -gt 0 ] && [ $1 = $cuenta ]; then
        ...
        ((contador++));
        if [ $encontro -eq 1 ] && [ $contador -gt $contar ]; then break; fi
    done
}
...
buscarCuenta(){
    ...
    while read line; do
        linea=$(echo -e "$line\n");
        cuenta=$(echo "$linea" | cut -d ';' -f2);
        if [ $contador -ne 0 ] && [ $1 = $cuenta ]; then
            if [ $2 -eq 0 ]; then
...
while read -r lin; do
    ...
    for line in $(cat $lin); do
        ...
        if [ $contador -ne 0 ]; then
            ...
            if [[ ! $fechadesde =~ ^[0-3][0-9]/[0-1][0-9]/[0-9]{4}$ ]]; then
                rechazados "ERROR: fecha con formato incorrecto"
                let aceptado=1;
            fi
            fechahasta="$aux/$aux2/$aux4";
            if [[ $fechahasta =~ ^[0-3][0-9]/[0-1][0-9]/[0-9]{4}$ ]]; then
